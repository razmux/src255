...
int npc_cashshop_buylist(struct map_session_data *sd, int points, int count, unsigned short* item_list)
{
	...
	{ // Cash Shop
		int value1 = pc_readregistry(sd,add_str(nd->u.shop.cash_var)),
			value2 = ( nd->u.shop.point_vartype != -1 ) ? pc_readregistry(sd,add_str(nd->u.shop.point_var)) : 0;

		if( value2 < points || value1 < (vt - points) )
			return 6;

		pc_setregistry(sd,add_str(nd->u.shop.cash_var),value1 - (vt - points));
		if( nd->u.shop.point_vartype != -1 ) pc_setregistry(sd,add_str(nd->u.shop.point_var),value2 - points);
	}
	// -------------------------------------------------------------------
...
int npc_cashshop_buy(struct map_session_data *sd, unsigned short nameid, int amount, int points)
{
	...
	{ // Cash Shop
		...
		pc_setregistry(sd,add_str(nd->u.shop.cash_var),value1 - (price - points));
		if( nd->u.shop.point_vartype != -1 ) pc_setregistry(sd,add_str(nd->u.shop.point_var),value2 - points);
	}
	// -------------------------------------------------------------------
...
int npc_unload(struct npc_data* nd, bool single) {
	...
	if( (nd->subtype == NPCTYPE_SHOP || nd->subtype == NPCTYPE_CASHSHOP || nd->subtype == NPCTYPE_ITEMSHOP || nd->subtype == NPCTYPE_POINTSHOP || nd->subtype == NPCTYPE_MARKETSHOP || nd->subtype == NPCTYPE_SPSHOP) && nd->src_id == 0) //src check for duplicate shops [Orcao]
	{
...
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	{// w1=<map name>,<x>,<y>,<facing>
		...
		case NPCTYPE_CASHSHOP:
			...
			break;
		case NPCTYPE_SPSHOP:
			if( sscanf(w1, "%15[^,],%6hd,%6hd,%4hd,%d", mapname, &x, &y, &dir, &cashitem) != 5
			||	strchr(w4, ',') == NULL )
			{
...
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	// get placement
	if( (type == NPCTYPE_SHOP || type == NPCTYPE_CASHSHOP || type == NPCTYPE_ITEMSHOP || type == NPCTYPE_POINTSHOP || type == NPCTYPE_SCRIPT || type == NPCTYPE_MARKETSHOP || type == NPCTYPE_SPSHOP) && strcmp(w1, "-") == 0 ) {// floating shop/chashshop/itemshop/pointshop/script
		x = y = dir = 0;
		m = -1;
	} else {
		char mapname[MAP_NAME_LENGTH_EXT];
...
int npc_parsesrcfile(const char* filepath, bool runOnInit)
{
	...
	for( p = skip_space(buffer); p && *p ; p = skip_space(p) )
	{
		...
		else if( (!strcasecmp(w2,"shop") || !strcasecmp(w2,"sshop") || !strcasecmp(w2,"cashshop") || !strcasecmp(w2,"itemshop") || !strcasecmp(w2,"pointshop") || !strcasecmp(w2,"marketshop") || (i = 0, sscanf(w2,"cashshop%n",&i), (i > 0 && w2[i] == '('))) && count > 3 )
